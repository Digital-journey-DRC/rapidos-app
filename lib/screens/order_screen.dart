...
class _CameraColisScreenState extends State<CameraColisScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller == null) {
      return const Scaffold(
        body: Center(child: Text('Aucune caméra disponible')),
      );
    }
    return Scaffold(
      body: Stack(
  ...
  Widget _buildClientOrders() {
    ...
    return StatefulBuilder(
      builder: (context, setState) {
        return Column(
          children: [
            // Filtre par statut pour le client
            Container(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: [
                  ...
                  return DropdownMenuItem<String>(
                    value: status,
                    child: Text(
                      status == 'Tous' ? 'Tous les statuts' : _translateStatus(status),
                      style: TextStyle(
                        color: status == 'Tous' ? Colors.grey : _statusColor(status),
                      ),
                    ),
                  );
                }).toList(),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            selectedStatus == 'Tous' 
                                ? 'Aucune commande trouvée'
                                : 'Aucune commande avec le statut "${_translateStatus(selectedStatus)}"',
  ...
  Widget _buildLivreurOrders() {
    ...
    return StatefulBuilder(
      builder: (context, setState) {
        return Column(
          children: [
            // Filtre par statut pour le livreur
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              decoration: BoxDecoration(
                color: Colors.white,
