...
class SelectPageTaxe extends StatefulWidget {
  const SelectPageTaxe({super.key, required this.apartmentId});
  final dynamic apartmentId;

  @override
  State<SelectPageTaxe> createState() => _SelectPageTaxeState();
}

class _SelectPageTaxeState extends State<SelectPageTaxe> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  double amountTaxe = 0.0;
  
  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this);
    print(widget.apartmentId['price']['amount']* 0.20);

    amountTaxe = widget.apartmentId['price']['amount']* 0.20;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          ...
          "L'appartement "+widget.apartmentId!['number'],
  ...
  Widget _buildDistributeInvoiceTab() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _buildSectionHeader('Nouvelle taxe à payer'),
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          "${amountTaxe.toStringAsFixed(2)} ${widget.apartmentId['price']['currency']}",
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                          ),
                        ...
                        Text(
                          ...
                          // widget.apartmentId,
                          'Represente 20% de la taxe sur la location',
                          style: TextStyle(
                            color: Colors.grey.shade600,
                            fontSize: 11,
                          ),
              ...
              const Divider(height: 32),
                          ...
                          // '${widget.building.address.city}, ${widget.building.address.country}',
                          // widget.apartmentId,
                          'Paiement du mois ${DateTime.now().month} / ${DateTime.now().year}',
                          style: const TextStyle(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                              ...
                              if (authState is! AuthSuccess || authState.token == null) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Vous devez être connecté pour effectuer le paiement'),
                                    backgroundColor: Colors.red,
                                  ),
                                );
                                return;
                              ...
                              try {
                                final response = await http.post(
                                  Uri.parse('http://68.183.30.146:8000/api/v1/taxes'),
                                  headers: {
                                    'Content-Type': 'application/json',
                                    'Authorization': 'Bearer ${authState.token}',
                                  },
                                  body: jsonEncode({
                                    'apartmentId': widget.apartmentId['_id'],
                                    'amount': {
                                      'value': amountTaxe,
                                      'currency': 'USD'
                                    },
                                    'dueDate': DateTime.now().toIso8601String(),
                                    'description': 'Paiement de la taxe locative'
                                  }),
                                );
